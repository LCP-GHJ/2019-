...
int BiAStar(Request request, vector<NodeData> open[2], vector<NodeData> close[2], CrossState **crossstate, bool isback)
{
	...
	for(int i = 0; i < cross_class[index_cross].idnex.size(); i++)
	{
		...
		//if (Find(close[isback], cross_next) != -1)
...
void GetAllRequestRoad(void)
{
	...
	while (deal_count < request[0][0])
	{
		...
		for(int i = 0; i < request[0][0]; i++)
		{
			...
			{
				FindRoad(request_class[i]);
				deal_count++;
				//cout << request_class[i].requestID << " ";
			}
		}
...
int main()
{
	...
	//	linkroad_use[1][i] = 0;
	//}
	//FindRoad(request_class[123]);
	//cout << request_class[123].cost << endl;
	//for (vector<int>::iterator it = request_class[123].road_cross.begin(); it != request_class[123].road_cross.end(); it++)
	//	cout << *it << " ";
